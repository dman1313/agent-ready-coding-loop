...
- **The builder handoff.** Once the plan is approved, the agent asks one last question: *which model will do the building?* It fetches that model's current documentation, gives you a plain-language digest plus the links so you can read it yourself, and records your advice **verbatim** in a Builder Briefing inside `ENGINE.md`. Whichever session builds — same model or a different one — must read the briefing first; your advice is binding, not a suggestion.
...
- **v3.2.1** — hardened by live dry-run tests (a fresh-start simulation and a resume simulation, each given the kit cold): Phase 2's agreed goals now persist as a "Version 1 — goals & scope" section in `MISSION.md` instead of living only in chat; `templates/` skeletons can never be mistaken for real phase outputs; an `ENGINE.md` older than a month triggers a docs re-check; the worked PYP example is marked illustration-only; the Builder Briefing only counts when filled in; the persistent-config instruction now says exactly which rules to paste.
- **v3.2** — **the builder handoff**: after the plan is approved, the agent asks which model will do the building, fetches that model's current documentation, hands the human a plain-language digest plus the doc links to read themselves, and records the human's advice verbatim as a binding Builder Briefing in `ENGINE.md`; any session that resumes as the builder must read it first, and a session that isn't the chosen builder must flag the mismatch before touching anything.
